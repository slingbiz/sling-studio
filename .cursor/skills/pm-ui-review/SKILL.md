...
- Create (`/create`) generates a page as 5 or 6 named sections in one AI call, streamed. Show a Streaming box with live lines immediately; paint each section on the page as it finishes. Hover is Studio chrome (`data-sling-section`): orange tile border + box shadow + label. The demo is one continuous page — wheel over it scrolls Studio, not an inner iframe. Process saves the same section code as draft widgets, one template, and a default route. Do not generate again on Process. Do not publish.
