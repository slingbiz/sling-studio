...
- Page templates landing (`/pages`) is tiles of the real storefront (capped live iframes), not a table of fake thumbs. Three tiles across on large screens. Default `pagelayout_default` image until the live preview loads or if it fails. Tile click opens layout. Edit/Delete stay visible. No Configure. Show the route path, not a URL count. No route = assign one. Do not iframe every template at once.
...
- Page templates list used the same generic png for every row. Tiles must iframe a real storefront route, capped like the widget library. Show the default png while the live preview loads or fails. Empty copy if there is no route. Three tiles across on large screens.
