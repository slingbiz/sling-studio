...
- Create progress is a Members-style modal with human status lines, not a terminal that says Streaming. Stop cancels generate. Process still saves drafts.
- Every AI page gets a unique kebab URL (`/moonbid`, `/moonbid-2`). Never assign `/` for a generated page. Template id can stay timestamped; the route must be a readable slug.
- Generated templates use the existing sm/md/lg grid. First and last widgets are full width. Middle widgets sit 2-up (6/6) or 3-up (4 on desktop, stack on mobile). Widget JSX must be responsive inside the cell (Grid + theme.breakpoints). Do not go back to a stack of 12/12/12 rows.
