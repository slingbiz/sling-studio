...
- Page templates list used the same generic png for every row. Tiles must iframe a real storefront route, capped like the widget library. Show the default png until the live page paints, not a white about:blank flash. Empty copy if there is no route. Three tiles across on large screens. Route path is a cream code chip. Edit/Delete are outline orange, not leftover text links.
