  ...
  const onDragEnd = (result) => {
    setTimeout(() => {
      ...
      if (sInd === dInd) {
        ...
        alert(
          'Moving a widget to a different block not allowed at the moment.',
        );
        // return;
        ...
        const toBeMoved =
          sectionBlocksSource.rows[posSource].cells[source.index];
    ...
    if (e.key === 'Enter') {
      const filteredWidgets = widgets.filter((widget) => {
        return (
          ...
          widget.key.toLowerCase().includes(query.toLowerCase())
        );
      });

      setSearchWidgets(filteredWidgets);

      // Ensure react-beautiful-dnd gets the updated list
      // Optionally trigger re-render to reset the drag context
    }
  };
